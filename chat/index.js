...
function sendMessage(message, username, uid, roomId) {
    ...
    try {
        con.query("INSERT INTO ?? (message, username, timestamp, roomid, uid) VALUES ( ?, ?,TIME_FORMAT(CURTIME(), '%h:%i:%s %p') CURDATE(), ?, ?, ?)", [nameString, msg, username,roomId, uid], function (error, results) {
            if (error) throw error
        })
    } catch (Exception) {
